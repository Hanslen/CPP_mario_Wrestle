...
void BaseEngine::CreateObjectArray(int iNumberObjects)
{
	...
	m_ppDisplayableObjects = new std::map < int, DisplayableObject*>() ;
...
void BaseEngine::StoreObjectInArray(int iIndex, DisplayableObject* pObject)
{
	...
	if (pObject != NULL){
		if (m_ppDisplayableObjects->find(iIndex) == m_ppDisplayableObjects->end()){
...
void BaseEngine::DestroyOldObjects()
{
	...
	if (m_ppDisplayableObjects != NULL){
		std::map<int, DisplayableObject*>::iterator iterator;
		for (iterator = m_ppDisplayableObjects->begin(); iterator != m_ppDisplayableObjects->end(); iterator++) {
...
void BaseEngine::UpdateAllObjects( int iCurrentTime )
{
	...
	if (m_ppDisplayableObjects != NULL){
		std::map<int, DisplayableObject*>::iterator iterator;
		for (iterator = m_ppDisplayableObjects->begin(); iterator != m_ppDisplayableObjects->end(); iterator++) {
				iterator->second->DoUpdate(iCurrentTime);
				if (m_iDrawableObjectsChanged){
					return;
				}
		}
...
void BaseEngine::UndrawObjects()
{
	...
	if (m_ppDisplayableObjects != NULL){
		std::map<int, DisplayableObject*>::iterator iterator;
		for (iterator = m_ppDisplayableObjects->begin(); iterator != m_ppDisplayableObjects->end(); iterator++) {
			iterator->second->RedrawBackground();
			if (m_iDrawableObjectsChanged){
				return;
			}
		}
...
void BaseEngine::NotifyAllObjects( int iSignalNumber )
{
	...
	if (m_ppDisplayableObjects != NULL){
		std::map<int, DisplayableObject*>::iterator iterator;
		for (iterator = m_ppDisplayableObjects->begin(); iterator != m_ppDisplayableObjects->end(); iterator++) {
			iterator->second->Notify(iSignalNumber);
		}
	}
}
...
int BaseEngine::NotifyAllObjectsGetCountNonZero( int iSignalNumber )
{
	...
	if (m_ppDisplayableObjects != NULL){
		std::map<int, DisplayableObject*>::iterator iterator;
		for (iterator = m_ppDisplayableObjects->begin(); iterator != m_ppDisplayableObjects->end(); iterator++) {
			if (iterator->second->Notify(iSignalNumber) != 0){
				iReturn++;
			}
		}
	}
...
int BaseEngine::NotifyAllObjectsGetSum( int iSignalNumber )
{
	...
	if (m_ppDisplayableObjects != NULL){
		std::map<int, DisplayableObject*>::iterator iterator;
		for (iterator = m_ppDisplayableObjects->begin(); iterator != m_ppDisplayableObjects->end(); iterator++) {
			iReturn += iterator->second->Notify(iSignalNumber);
		}
	}
	return iReturn;
}
...
int BaseEngine::NotifyAllObjectsGetMax( int iSignalNumber )
{
	...
	if (m_ppDisplayableObjects != NULL){
		std::map<int, DisplayableObject*>::iterator iterator;
		for (iterator = m_ppDisplayableObjects->begin(); iterator != m_ppDisplayableObjects->end(); iterator++) {
			int ival = iterator->second->Notify(iSignalNumber);
			if (ival > iReturn){
				iReturn = ival;
			}
		}
